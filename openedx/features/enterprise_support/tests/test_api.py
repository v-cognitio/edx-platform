...
class TestEnterpriseApi(EnterpriseServiceMockMixin, CacheIsolationTestCase):
    ...
    def test_get_enterprise_learner_data_from_db(self):
        EnterpriseCustomerUserFactory(user_id=self.user.id)  # lint-amnesty, pylint: disable=unused-variable
    ...
    def test_get_data_sharing_consents_from_db(self, is_enterprise_enabled, mock_enterprise_enabled):
        mock_enterprise_enabled.return_value = is_enterprise_enabled
        enterprise_customer_user = EnterpriseCustomerUserFactory(user_id=self.user.id)

        if not is_enterprise_enabled:
            assert get_data_sharing_consents_from_db(self.user) == []
        else:
            course_id = 'fake-course'
            data_sharing_consent = DataSharingConsent(
                course_id=course_id,
            ...
            data_sharing_consent.save()
            data_sharing_consents = get_data_sharing_consents_from_db(self.user)
            assert len(data_sharing_consents) == 1
            assert data_sharing_consents[0].id == data_sharing_consent.id

    @ddt.data(True, False)
    @mock.patch('openedx.features.enterprise_support.api.enterprise_enabled')
    def test_get_enterprise_course_enrollments_from_db(self, is_enterprise_enabled, mock_enterprise_enabled):
        mock_enterprise_enabled.return_value = is_enterprise_enabled
        enterprise_customer_user = EnterpriseCustomerUserFactory(user_id=self.user.id)

        if not is_enterprise_enabled:
            assert get_enterprise_course_enrollments_from_db(self.user) == []
        else:
            ece = EnterpriseCourseEnrollmentFactory(enterprise_customer_user=enterprise_customer_user)
            enterprise_course_enrollments = get_enterprise_course_enrollments_from_db(self.user)
